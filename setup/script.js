...
// load items
window.addEventListener('DOMContentLoaded', back);
function back () {
  menuDisplay(menu);
  btnDisplay();
};
...
function menuDisplay(menuItems) {
  let newArray = menuItems.map(mapFunc);
...
function btnDisplay() {
  const categories = menu.reduce(
    ...
    const categoryBtn = categories.map(catFunc).join('');
    ...
    // filter items
filterBtn.forEach(callBack);
function callBack(btn) {
  btn.addEventListener('click', myEvent);
}
function myEvent(e) {
  const category = e.currentTarget.dataset.id;
  const menuCategory = menu.filter(filterMenu);
